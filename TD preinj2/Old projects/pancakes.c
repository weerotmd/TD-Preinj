...
Pancake* queuePancake(Pancake* queue, Pancake pancake) {
  ...
  if (queue == NULL) return newPancake;
  Pancake* pos = queue;
  while (pos->next != NULL) {
    pos = pos->next;
  }
...
int biggestPancakeEver(Pancake* stack, int n) {
  Pancake* pos = stack;
  if (pos == NULL) return -1;
  int biggest = pos->size;
  int biggestPos = 1;
  int i = 1;
  while (pos->next != NULL && i<n) {
    pos = pos->next;
    i++;
    if (pos->size > biggest) {
      biggestPos = i;
...
Pancake* invertPancakesCopy(Pancake* stack, int n) {
  if (stack == NULL) return stack;
...
Pancake* invertPancakes(Pancake* stack, int n) {
  ...
  while (stack != NULL && i < n) {
    int newPancake;
    stack = shiftPancake(stack, &newPancake);
    queue = queuePancake(queue, makePancake(newPancake));
    i++;
  }
  while (queue != NULL) {
    int newPancake;
    queue = shiftPancake(queue, &newPancake);
    stack = stackPancake(stack, makePancake(newPancake));
  }
  return stack;
}
Pancake* invertBiggest(Pancake* stack, int n) {
  return invertPancakes(stack, biggestPancakeEver(stack, n));
}
...
Pancake* sortStack(Pancake* stack) {
  ...
  while (pancakes > 1 && !pancakeStackOrdered(stack)) {
    stack = invertBiggest(stack, pancakes);
    displayPancakeStack(stack);
    stack = invertPancakes(stack, pancakes);
    displayPancakeStack(stack);
    pancakes--;
  }
  return stack;
